...
btn_ligas.forEach((el)=>{
    el.addEventListener("click",(evt)=>{
        ...
        .then(res=>{
            ...
            // console.log(response.data.standings[0].stats) // pegar gamesplayed losses, pointDifferences, points, wins
            // console.log(response.data.standings[0].team) // pegar o abbreviation e logo
...
btnVoltar.addEventListener("click",()=>{
    // const tabela_tbody = document.querySelector("#tabela_tbody") // parei aqui 
    // tabela_tbody.remove()
...
const createTable=(time)=>{
    ...
    const tabelaLiga = document.querySelector("#tabelaLiga")

    const tbody = document.createElement("tbody")
    tbody.setAttribute("id","tabela_tbody")
    tabelaLiga.appendChild(tbody)
    ...
    img_tdImgName.setAttribute("src",time.team.logos[0])
